  ...
  constructor(props) {
    ...
    var requestColumns = [
      ...
      },
      {
        ...
        render: (text, record) => (
          <span>
            <Button type="primary" onClick={this.requestConfirm.bind(this)}>
              Request Book
            </Button>
          </span>
        ),
    ...
    var returnColumns = [
      ...
      // },
      {
        ...
        render: (text, record) => (
          <span>
            <Button type="primary" onClick={this.returnConfirm.bind(this)}>
              Return Book
            </Button>
          </span>
        ),
  ...
  handleOk = (e) => {
    ...
    this.state.updatedDataSource.forEach(function (dataSource) {
      if (dataSource.key.toLowerCase() === selectedKey) {
        if (dataSource.availableItems === 0) allowUpdate = false;
        dataSource.availableItems = allowUpdate
          ? dataSource.availableItems - 1
          : 0;
        if (allowUpdate) updatedMyBooksDataSource.push(dataSource);
        else {
          alert("Out of stock");
          return false;
        }
    ...
    if (allowUpdate) {
      this.setState({
        updatedDataSource: updatedBooksDataSource,
        myBooksDataSource: updatedMyBooksDataSource,
        selectedRowKeys: [],
        updatedMyBooksDataSource: updatedMyBooksDataSource,
      });
  ...
  filterDataSource(value, tabName) {
    ...
    if (tabName === "requestbooks") {
      defaultDataSource.forEach(function (dataSource) {
        if (
          dataSource[selectedSearchType]
            .toString()
            .toLowerCase()
            .indexOf(valueLowerCase) !== -1
        )
